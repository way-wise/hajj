...
const HeaderTheme01: React.FC<HeaderTheme01Props> = ({ header }) => {
  ...
  return (
    <header>
      <div className="header left-0 top-0 z-40 sticky shadow-sticky backdrop-blur-sm transition">
        {header?.callback && (
          <div className='xl:hidden bg-black/90 backdrop-blur py-1.5'>
            <div className='container'>
              <div className='xl:hidden flex justify-between gap-4'>
                {header?.callback.split(',').map((substring, idx) => {
                  return (
                    <div key={idx}>
                      ...
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-telephone-forward size-4" viewBox="0 0 16 16">
        ...
        <div className='bg-white !bg-opacity-80 py-5'>
          <div className="container">
            <div className="relative -mx-4 flex items-center justify-between">
              ...
              <div className="flex w-full items-center justify-end px-4 gap-6">
                        ...
                        {menuItem.subNavItems && menuItem.subNavItems.length === 0 ? (
                          <>
                            <CMSLink
                              {...menuItem?.link}
                              appearance="link"
                              className={`font-medium block py-2 text-base lg:mr-0 lg:px-0 lg:py-6 no-underline ${pathname === menuItem?.link?.url
                ...
                <button onClick={navbarToggleHandler} className="xl:hidden">
                  <span className="my-1.5 block h-0.5 w-[30px] bg-black" />
                  <span className="my-1.5 block h-0.5 w-[30px] bg-black" />
                  <span className="my-1.5 block h-0.5 w-[30px] bg-black" />
                </button>
              </div>
      ...
      {/* mobile navbar */}
      <div className={`xl:hidden fixed top-0 end-0 w-full max-w-[320px] h-screen bg-black/50 backdrop-blur z-50 px-4 py-10 text-white transition-all duration-500 transform ${navbarOpen ? 'translate-x-0' : 'translate-x-full'}`}>
        <div className="flex justify-between items-center px-5 rounded-lg bg-white/50 mb-8">
          <Link href="/" className={`header-logo block w-full max-w-[150px] py-6 lg:py-3`}>
            {header?.logo && (
              <Media resource={header?.logo} className="w-44 sm:w-52 md:w-56 lg:max-w-60 h-auto dark:brightness-200" priority />
            )}
          </Link>
          <button onClick={navbarToggleHandler}>
            <svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="bi bi-x-circle-fill size-6" viewBox="0 0 16 16">
                ...
                {menuItem.subNavItems && menuItem.subNavItems.length === 0 ? (
                  <>
                    <CMSLink
                      {...menuItem?.link}
                      appearance="link"
                      className={`font-medium block px-3 py-2 text-base no-underline ${pathname === menuItem?.link?.url ? 'text-primary' : 'text-white/90'
                        }`}
                    />
                  </>
                ) : (
                  <>
                    ...
                    <div
                      className={`submenu relative start-5 top-full rounded-sm bg-transparent transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark ${openIndex === index ? 'block' : 'hidden'
                        }`}
                    >
                      {menuItem.subNavItems &&
                        menuItem.subNavItems.map((submenuItem, subIndex) => (
      ...
      {navbarOpen && (
        <div className='fixed top-0 start-0 bg-black/50 h-screen w-full z-40' onClick={() => setNavbarOpen(false)}></div>
      )}
    </header >
  )
}
