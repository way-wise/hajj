                                    ...
                                    {menuItem.subNavItems && menuItem.subNavItems.length === 0 ? (
                                        <>
                                            <CMSLink
                                                {...menuItem?.link}
                                                appearance="link"
                                                className={`font-medium flex justify-start px-5 py-2 text-base no-underline ${pathname === menuItem?.link?.url
                                        ...
                                        <>
                                            ...
                                            <div
                                                className={`submenu rounded-sm bg-white z-[999] transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark absolute top-full start-20 ${openIndex === index ? 'visible' : 'invisible'
                                                    }`}
                                            >
                                    ...
                                    {menuItem.subNavItems && menuItem.subNavItems.length === 0 ? (
                                        <>
                                            <CMSLink
                                                {...menuItem?.link}
                                                appearance="link"
                                                className={`font-medium flex py-2 text-base lg:mr-0 lg:!inline-flex lg:!px-0 lg:!py-3 no-underline ${pathname === menuItem?.link?.url
                                                    ? 'text-primary dark:text-white'
                                                    : 'text-dark hover:text-primary dark:text-white/70 dark:hover:text-white'
                                                    }`}
                                            />
                                        ...
                                        <>
                                            ...
                                            <div
                                                className={`submenu relative left-0 top-full rounded-sm bg-white z-[999] transition-[top] duration-300 group-hover:opacity-100 dark:bg-dark lg:invisible lg:absolute lg:top-[110%] lg:block lg:w-[280px] lg:p-4 lg:opacity-0 lg:shadow-lg lg:group-hover:visible lg:group-hover:top-full ${openIndex === index ? 'block' : 'hidden'
                                                    }`}
                                            >
                                                {menuItem.subNavItems &&
                                                    menuItem.subNavItems.map((submenuItem, index) => (
                    ...
                    <div className='flex gap-3 items-center'>
                        {header?.callback &&
                            header?.callback.split(',').map((substring, idx) => {
                                return (
                                    <a href={`tel:${substring}`} className='inline-flex gap-1.5 items-center font-medium text-xs sm:text-base' key={idx}>
                ...
                <div className='container'>
                    <div className='py-2 flex justify-between items-center'>
                        ...
                        <div className='flex gap-4 items-center'>
                            ...
                            <a href="#" className='hidden sm:inline-flex justify-center items-center bg-white px-4 py-2.5 rounded-lg text-black'>Contact Us</a>
                        </div>
                    </div>
                </div>
            </div>
