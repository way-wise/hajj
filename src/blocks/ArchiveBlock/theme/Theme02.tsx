                ...
                {posts.length > 0 &&
                    posts.map((post, index) => {
                        if (typeof post === 'object' && post !== null) {
                            return post?.slug ? (
                                <a className="group block rounded-xl overflow-hidden focus:outline-none" href={`/posts/${post.slug}`} key={index}>
                                    <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-5">
                                        {post?.meta?.image && (
                                            <div className="shrink-0 relative rounded-xl overflow-hidden w-full sm:w-56 h-44">
                                                <Media resource={post?.meta?.image} imgClassName="group-hover:scale-105 group-focus:scale-105 transition-transform duration-500 ease-in-out size-full absolute top-0 start-0 object-cover rounded-xl" />
                                            </div>
                                        )}
                                        <div className="grow">
                                            ...
                                            <p className="mt-4 inline-flex items-center gap-x-1 text-sm text-blue-600 decoration-2 group-hover:underline group-focus:underline font-medium dark:text-blue-500">
                                                Read more
                                                <svg className="shrink-0 size-4" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" stroke-linejoin="round"><path d="m9 18 6-6-6-6" /></svg>
                                            </p>
                                        </div>
                                    </div>
                                </a>
