                            ...
                            return (
                                <div className="p-4 xl:w-1/4 md:w-1/2 w-full" key={index}>
                                    <div className="h-full p-6 rounded-lg border-2 border-gray-300 dark:border-gray-700 flex flex-col relative overflow-hidden">
                                        {highlightText && <span className="bg-indigo-500 text-white px-3 py-1 tracking-widest text-xs absolute right-0 top-0 rounded-bl">{highlightText}</span>}

                                        <h2 className="text-sm tracking-widest title-font mb-1 font-medium">{title}</h2>
                                        <h1 className="text-5xl text-gray-900 dark:text-gray-300 pb-4 mb-4 border-b border-gray-200 leading-none">{price} / <span className='text-xl'>{priceType ? 'Year' : 'Month'}</span></h1>
                                        {
                                            features && (
                                                features.map((feature, index) => {
                                                    return (
                                                        <p className="flex items-center text-gray-600 dark:text-gray-300 mb-2" key={index}>
                                                            ...
                                                            </span><span>{feature.feature}</span>
                                                        </p>
                                                    )
                                                })
                                            )
