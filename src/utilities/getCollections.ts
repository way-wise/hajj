...
async function getDocument({ collection, where = {}, depth = 0 }: GetDocArgs) {
    ...
    return page.docs // return all documents or filtered document(s)
}
...
export const getCollections = ({ collection, where = {}, cached, depth }: Args) => {
    if (cached) {
        return unstable_cache(
            async () => getDocument({ collection, where, depth }),
        ...
        return async () => getDocument({ collection, where, depth })
    }
}
